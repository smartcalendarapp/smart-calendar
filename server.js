...
app.use(express.static(path.join(__dirname, 'public', 'researchassets')))
...
app.post('/cancelsubscription', async (req, res) => {
	try{
		...
		if(reason){
			...
			const message = new FeedbackObject({ userid: userid, content: 'Subscription cancelled.\n' + reason })
...
let MAX_GPT_CHAT_PER_DAY = 40 //20
...
let MAX_GPT_COMPLETION_PER_DAY = 40 //10
...
app.post('/getcardhint', async (req, res) => {
	...
	const systemprompt = showanswer ? [`Your task is to provide a seamless memory loci narrative. Any confusing terms must be made intuitively clear. 2-3 sentences max. Use name word-play and word-roots (and words with shared roots) to make more memorable`, `Your task is to first define all complex terminology in a few words. Then, explain the concept like the user is five years old. Use name word-play, word-roots (refer to words with shared roots), and/or association linking to make more understandable. 2-3 sentences max.`][hinttype] : `Your task is to use association linking to help user recall the back without revealing the answer`
