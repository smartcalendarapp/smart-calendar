...
class Calendar {
	...
	updateSettings() {
		...
		if (calendar.settings.issyncingtogooglecalendar) {
			...
			let text = `
			<div class="text-14px text-primary">Last synced ${calendar.lastsyncedgooglecalendardate ? `${getRelativeDHMText(difference)}` : 'never'}.</div>`
...
function updatecalendaritempopup(id) {
	...
	calendaritempopup.innerHTML = `
		...
		<div class="info">
			...
			${item.googleid ? `<div class="text-blue text-14px">Syncing with Google Calendar</div>` : ''}
				...
				`<div class="pre-wrap break-word selecttext todoitemtext text-quaternary text-14px overflow-hidden ${itemclasses.join(' ')}">${formatURL(cleanInput(item.notes))}</div>` : ''}
								...
								<div class="display-flex flex-wrap-wrap flex-row align-center column-gap-12px row-gap-6px">
									...
									<div class="gap-6px todoitemtextbutton display-flex flex-row align-center width-fit todoitemtext nowrap pointer-none popupbutton" onclick="clicktodoitemduedate(event, '${item.id}')">
										<div class="pointer-auto pointer ${!endbeforedate ? 'text-quaternary hoverunderlinequaternary' : (isoverdue ? 'text-red hoverunderlinered' : 'text-blue hoverunderlineblue')} text-14px ${itemclasses.join(' ')}">${endbeforedate ? `Due ${CgetHMText(item.endbefore.minute)}` : 'No due date'}</div>
									</div>
