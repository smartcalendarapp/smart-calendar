...
async function autoScheduleV2(smartevents, addedtodos, resolvedpassedtodos) {
	...
	function getconflictingevent(data, item1) {
		...
		for (let item2 of sortdata) {
			...
			console.log(tempstartdate1 + '', new Date(tempenddate2.getTime() + spacing).getTime(), new Date(tempenddate2 + spacing).getTime(), tempstartdate2+'', tempstartdate1.getTime() < tempenddate2.getTime() + spacing && tempenddate1.getTime() + spacing > tempstartdate2.getTime())
