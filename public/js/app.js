...
function getDate(string = '') {
	...
	let tempdate12 = string.match(/\b((\d{4})(\/|-)(0?[1-9]|1[0-2])(\/|-)(0?[1-9]|1[0-9]|2[0-9]|3[0-1]))\b/)
	if (tempdate12) {
		match = tempdate12[0]
...
class Calendar {
	...
	updateUpgrade(){
		...
		let upgradeprogressitem3 = getElement('upgradeprogressitem3')
		let upgradeprogressitem4 = getElement('upgradeprogressitem4')

		upgradeprogressitem1.classList.remove('selectedupgradeprogressitem')
		upgradeprogressitem2.classList.remove('selectedupgradeprogressitem')
		...
		let displaytext = `Your plan: ${clientinfo?.subscriptionstatus == 'ongoing' ? 'Renews' : 'Expires'} ${getRelativeDHMText(Math.floor(Date.now() - clientinfo?.premiumendtimestamp)/60000)}`
...
//update time
let lastupdateminute = new Date().getHours() * 60 + new Date().getMinutes()
...
function run() {
	...
	*/
	...
	setInterval(async function(){
		...
		if(currentdate.getHours() * 60 + currentdate.getMinutes() != templastupdateminute){
...
function updateonboardingscreen(){
	...
	}else if(!calendar.onboarding.choosecalendars){
		currentonboarding = 'choosecalendars'
	...
	}else if(currentonboarding == 'choosecalendars'){
		let settingscalendarlist = getElement('settingscalendarlist')
...
function continueonboarding(key){	
	...
	if(key == 'connectcalendars'){
		if(!calendar.settings.issyncingtogooglecalendar || calendar.calendars.length == 1){
			continueonboarding('choosecalendars') //skip
		}
	}
...
const SPEECH_ALWAYS_ON_TIMEOUT_DURATION = 10000
const RECOGNITION_CALL_PHRASE = 'athena'
...
function updateaichat(){
	...
	for(let chatinteraction of chathistory.getInteractions()){
		for(let { role, message, displaycontent, actions, id, liked, disliked, finishedanimating, waitingforvoice, nextactions, unread, iscommand, link } of chatinteraction.getMessages()){
			if(unread){
				unread = false
			}
			...
			<div class="aichatmessagewrap display-flex flex-column gap-12px">
				...
				<div class="flex-1 overflow-hidden display-flex flex-column gap-12px">
					<div class="display-flex flex-column gap-6px">
						...
						${link ? `<a class="align-self-center text-decoration-none padding-6px-12px display-flex flex-row align-center gap-6px border-8px background-blue hover:background-blue-hover transition-duration-100 text-white text-14px" href="${link.url}" target="_blank" rel="noopener noreferrer">${link.text}<svg height="100%" stroke-miterlimit="10" style="fill-rule:nonzero;clip-rule:evenodd;stroke-linecap:round;stroke-linejoin:round;" viewBox="0 0 256 256" width="100%" class="buttonwhitefill">
						<g>
						<g opacity="1">
						<path d="M239.757 16.3818C232.631 9.25524 224.162 4.74331 214.351 2.84599C204.541 0.948662 193.712 0 181.866 0L73.718 0C62.0564 0 51.3435 0.948662 41.5792 2.84599C31.8149 4.74331 23.3695 9.25524 16.243 16.3818C9.20897 23.4158 4.74331 31.7918 2.84599 41.5098C0.948662 51.2278 0 61.9176 0 73.5792L0 180.755C0 192.602 0.925524 203.407 2.77657 213.171C4.62762 222.936 9.11641 231.335 16.243 238.369C23.3695 245.495 31.8149 250.007 41.5792 251.905C51.3435 253.802 62.149 254.751 73.9957 254.751L181.866 254.751C193.712 254.751 204.541 253.802 214.351 251.905C224.162 250.007 232.631 245.495 239.757 238.369C246.791 231.335 251.257 222.936 253.154 213.171C255.051 203.407 256 192.602 256 180.755L256 73.9957C256 62.149 255.051 51.3435 253.154 41.5792C251.257 31.8149 246.791 23.4158 239.757 16.3818ZM233.649 70.1085L233.649 184.503C233.649 191.722 232.978 198.733 231.636 205.536C230.294 212.338 227.54 217.868 223.375 222.126C219.21 226.291 213.68 229.044 206.785 230.386C199.89 231.728 192.879 232.399 185.753 232.399L70.2473 232.399C63.1208 232.399 56.1099 231.728 49.2148 230.386C42.3196 229.044 36.7433 226.291 32.4859 222.126C28.321 217.868 25.5907 212.338 24.295 205.536C22.9993 198.733 22.3514 191.722 22.3514 184.503L22.3514 70.5249C22.3514 63.2133 22.9993 56.1099 24.295 49.2148C25.5907 42.3196 28.321 36.7896 32.4859 32.6247C36.6508 28.3673 42.227 25.5907 49.2148 24.295C56.2025 22.9993 63.3521 22.3514 70.6638 22.3514L185.753 22.3514C192.879 22.3514 199.89 23.0224 206.785 24.3644C213.68 25.7064 219.21 28.4599 223.375 32.6247C227.633 36.8821 230.409 42.4121 231.705 49.2148C233.001 56.0174 233.649 62.9819 233.649 70.1085ZM171.592 161.458C174.646 161.458 177.099 160.416 178.95 158.334C180.801 156.252 181.727 153.591 181.727 150.351L181.727 84.9631C181.727 80.7983 180.685 77.8366 178.603 76.0781C176.521 74.3196 173.721 73.4403 170.204 73.4403L104.26 73.4403C101.021 73.4403 98.4295 74.3659 96.4859 76.2169C94.5423 78.068 93.5705 80.5206 93.5705 83.5748C93.5705 86.7216 94.5654 89.2205 96.5553 91.0716C98.5452 92.9226 101.206 93.8482 104.538 93.8482L129.805 93.8482L150.351 91.6269L128.833 111.757L77.6052 162.985C75.3839 165.206 74.2733 167.659 74.2733 170.343C74.2733 173.49 75.1988 176.012 77.0499 177.909C78.9009 179.806 81.3536 180.755 84.4078 180.755C86.1663 180.755 87.6934 180.454 88.9892 179.852C90.2849 179.251 91.3955 178.487 92.321 177.562L143.549 126.334L163.401 105.093L161.319 126.612L161.319 150.629C161.319 153.961 162.268 156.599 164.165 158.542C166.062 160.486 168.538 161.458 171.592 161.458Z" fill-rule="nonzero" opacity="1" ></path>
						</g>
						</g>
						</svg>
						</a>` : ''}
						...
						<div class="display-flex flex-row gap-6px align-self-center ${chathistory.getInteractions().findIndex(d => d.id == chatinteraction.id) == chathistory.getInteractions().length - 1 ? '' : 'visibility-hidden small:visibility-visible hoverchatmessagebuttons'} ${!finishedanimating ? 'display-none' : ''}">
							...
							<div class="pointer aimessagebutton pointer-auto padding-6px ${disliked ? 'background-tint-1 border-8px' : ''}"  onclick="dislikeaichatmessage(event, '${id}')">
								<svg height="100%" stroke-miterlimit="10" style="fill-rule:nonzero;clip-rule:evenodd;stroke-linecap:round;stroke-linejoin:round;" viewBox="0 0 256 256" width="100%" class="buttonfillquaternarysmall pointer-none">
								<g>
	...
	if(calendartabs.includes(4)){
		for(let chatinteraction of chathistory.getInteractions()){
			for(let chatmessage of chatinteraction.getMessages()){
				...
				if(chatmessage.link && !chatmessage.link.clicked){
					chatmessage.link.clicked = true

					try{
						window.open(chatmessage.link.url)
					}catch(err){}
				}
			}
		}
					...
					if(command == 'create_event'){
						...
						if(error && !title && !startminute && !startyear){
							...
							if(startdate && !isNaN(startdate.getTime()) && enddate && !isNaN(enddate.getTime())){
								...
								responsechatmessage.message = ((responsechatmessage.message && responsechatmessage.message + '\n') || '') + `Done! I created an event "${Calendar.Event.getRawTitle(item)}" in your calendar for ${Calendar.Event.getStartText(item)}${item.repeat.frequency != null && item.repeat.interval != null ? `, repeating ${getRepeatText(item, true)}` : ''}.`
								...
								//UI to ask for time if not provided
								...
								responsechatmessage.nextactions = [...timeoptions.map(d => `<div class="hover:background-tint-1 bordertertiary border-8px transition-duration-100 pointer text-primary text-14px padding-8px-12px" onclick="promptaiassistantwithnextaction('${d}')">${d}</div>`), `<div class="hover:background-tint-1 bordertertiary border-8px transition-duration-100 pointer text-primary text-14px padding-8px-12px" onclick="promptaiassistantwithnextaction('Make it a task')">Make it a task (auto)</div>`]
							}
						}
...
async function autoScheduleV2({smartevents = [], addedtodos = [], resolvedpassedtodos = [], eventsuggestiontodos = [], moveditemtimestamp, moveditem}) {
	try{
		...
		let doingtodos = sortstartdate(smartevents).filter(d => new Date(d.start.year, d.start.month, d.start.day, 0, d.start.minute).getTime() <= Date.now() && new Date(d.end.year, d.end.month, d.end.day, 0, d.end.minute).getTime() > Date.now() && !getconflictingevent(iteratedevents, d) && (!moveditem || d.id != moveditem.id) && !addedtodos?.find(f => f.id == d.id))
		if(doingtodos[0]){
...
async function clickupgrade(option){
	try {
		if(clientinfo?.haspremium && clientinfo?.premiumplan != null){
			...
			if(response.status == 401){
				let data = await response.json()
				displaypopup(`<div class="display-flex flex-column gap-6px"><div class="text-primary text-18px">Could not load payment page</div><div class="text-quaternary text-16px">An error occured: ${data.error}. Try signing in again or <a href="../contact" class="text-decoration-none text-blue width-fit pointer hover:text-decoration-underline" target="_blank">contact us</a>.</div><div class="pointer-auto text-16px bluebutton text-center gap-6px padding-6px-12px border-8px transition-duration-100 pointer" onclick="closepopup(event)">Ok</div></div>`)
			}else{
				const session = await response.json();
