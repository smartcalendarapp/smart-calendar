...
async function likeaichatmessage(event, id){
	...
	try{
		const response = await fetch(`/sendmessage`, {
			...
			body: JSON.stringify({
				content: {
					type: 'AI Chat Message - LIKE',
					messages: chatinteraction.getMessages()
				}
			})
		})
...
function updateaichat(){
	...
	for(let chatinteraction of chathistory.getInteractions()){
		for(let { role, displaycontent, actions, id, liked, disliked, finishedanimating } of chatinteraction.getMessages()){
			output.push(`
			<div class="display-flex flex-row gap-12px">
				...
				<div class="overflow-hidden display-flex flex-column gap-12px">
					...
					${role == 'system' ? `
					<div class="display-flex flex-row gap-6px ${!finishedanimating ? 'display-none' : ''}" id="chatmessage-liked-${id}">
						<svg height="100%" stroke-miterlimit="10" style="fill-rule:nonzero;clip-rule:evenodd;stroke-linecap:round;stroke-linejoin:round;" viewBox="0 0 256 256" width="100%" class="aimessagebutton padding-6px ${liked ? 'background-tint-1' : ''}" onclick="likeaichatmessage(event, '${id}')">
						<g>
						...
						<svg height="100%" stroke-miterlimit="10" style="fill-rule:nonzero;clip-rule:evenodd;stroke-linecap:round;stroke-linejoin:round;" viewBox="0 0 256 256" width="100%" class="aimessagebutton padding-6px ${disliked ? 'background-tint-1' : ''}" onclick="dislikeaichatmessage(event, '${id}')">
						<g>
					...
					${actions ? `<div class="display-flex flex-row gap-12px flex-wrap-wrap ${!finishedanimating ? 'display-none' : ''}"  id="chatmessage-actions-${id}">${actions.join('')}</div>` : ''}
				</div>
			</div>`)
		}
	}
