		...
		for(let { role, message, displaycontent, actions, id, liked, disliked, finishedanimating } of chatinteraction.getMessages()){
			...
			<div class="display-flex flex-row gap-12px">
				...
				<div class="overflow-hidden display-flex flex-column gap-12px">
					...
					${role == 'system' ? `
					<div class="display-flex flex-row gap-6px ${chathistory.getInteractions().findIndex(d => d.id == chatinteraction.id) == chathistory.getInteractions().length - 1 ? '' : 'visibility-hidden small:visibility-visible hoverchatmessagebuttons'} ${!finishedanimating ? 'display-none' : ''}" id="chatmessage-liked-${id}">
			...
			if(output.command){
				if(output.command == 'create_event'){
					...
					if(startdate && !isNaN(startdate.getTime()) && enddate && !isNaN(enddate.getTime())){
						...
						responsechatmessage.message = `Done! I have created an event "${cleanInput(Calendar.Event.getTitle(item))}" in your calendar for ${Calendar.Event.getStartText(item)}.` + `\n\nTokens: ${data.data?.totaltokens}`
					...
					if(error){
						...
						if(item){
							if(Calendar.Event.isReadOnly(item)){
								...
								responsechatmessage.message = `Done! I have deleted your event "${cleanInput(Calendar.Event.getTitle(item))}."\n\nTokens: ${data.data?.totaltokens}`
							}
						}else{
							responsechatmessage.message = `I could not find that event, could you please tell me more?` + `\n\nTokens: ${data.data?.totaltokens}`
						}
					...
					if(error){
						...
						if(item){
							...
							responsechatmessage.message = `Done! I have deleted your task "${cleanInput(Calendar.Event.getTitle(item))}."\n\nTokens: ${data.data?.totaltokens}`
						}else{
							responsechatmessage.message = `I could not find that task, could you please tell me more?` + `\n\nTokens: ${data.data?.totaltokens}`
						}
					}
